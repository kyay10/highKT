...
typealias StateAlias<A, S> = State<S, A>
typealias StateIdentity<S, A> = K2<Constructor<StateAlias<*, *>>, A, S>
...
suspend fun stateMonadAliased() {
    data class CounterState(val count: Int)

    val incrementCounter: StateAlias<Unit, CounterState> = suspend { state: CounterState ->
        Unit to state.copy(count = state.count + 1)
    }

    val doubleCounter: StateIdentity<CounterState, Unit> = suspend { state: CounterState ->
