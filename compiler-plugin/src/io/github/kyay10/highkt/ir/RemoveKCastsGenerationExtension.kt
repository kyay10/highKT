...
private val IrType.isK: Boolean
  get() = erasedUpperBound.kotlinFqName == K_FQNAME || erasedUpperBound.hasAnnotation(
    TYPE_LAMBDA_FQNAME
  )
private val IrType.containsK: Boolean
  get() = isK || (this as? IrSimpleType)?.arguments?.any {
    it is IrType && it.containsK
  } == true
...
class FixupOverriddenFunctionsVisitor(pluginContext: IrPluginContext) : IrVisitorVoid() {
  val typeSystemContext =
    if (pluginContext.platform.isJvm()) JvmIrTypeSystemContext(pluginContext.irBuiltIns) else IrTypeSystemContextImpl(
  ...
  override fun visitClass(declaration: IrClass) {
    declaration.functions.groupBy { it.name }.forEach { (_, functions) ->
      ...
      for (realFunction in realFunctions) {
        ...
        for (fakeOverride in fakeOverrides) {
          ...
          fakeOverride.nonDispatchParameters.zip(realFunction.nonDispatchParameters).forEach { (fakeParameter, realParameter) ->
        ...
        realFunction.nonDispatchParameters.zip(parameterTypes).forEach { (parameter, type) ->
          parameter.type = type
        }
